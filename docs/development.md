...
動く絵も同じ広さで動かし、貼るときに 1120×630 へ縮める) は
...
終わるまで) が大半です。ワーカーを増やしても、いちばん長いファイル1本の時間が
1台での下限です。

CI では**台を増やして**います。4つに割って別のランナーに出すので、
3ワーカー×4台 = 12ワーカーぶん並びます。**組の中身は `tests/e2e/shards.json`** です。
...
CI のランナー (4コア) では1つ空けます。4つ並べると、チューナーを掴むのを待つテストが
...
`json(名前, 読み手)` で書く。読むときに解いて書くときに畳むので、使う側は配列や
...
追っていた頃は、同じコミットから焼いても中身が違いえました。**「実機に何が
入っているか」を知るには Pod へ入るしかありませんでした。CM検出の3つ (`logoframe`・
