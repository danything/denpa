...
| 口 | 守り方 |
| --- | --- |
| `/api/recordings/<id>/file` | **期限付きの署名リンク** (`?token=…`、`share.ts`)。OIDC で入った画面のぶんは**ログインの控えも受ける** |
...
普通の HTTP** なので、OIDC も信頼した網もそのまま効きます。1回使ったら
...
来る口は、ログインではなく **URL そのものが資格になる署名リンク**で開けます —
...
古いDBに残っている `basicAuthUser` などの設定値は読みません (残っていても無害)。
