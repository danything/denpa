...
来る口は、ログインではなく URL そのものが資格になる期限付きリンクで開けます。
「テレビで再生」も「再生リンクをコピー」もダウンロードも、押した瞬間に
24時間で切れるリンクを作って渡します (`share.ts`)。

**1録画につき現役のリンクは1本**で、控えをDB (`share_links`) に持ちます。
期限内にもう一度押すと、同じURLのまま期限だけ延びます。テレビの履歴に残った
URLは、使い続けているかぎり切れません。全部を今すぐ切りたければ
`share_links` の行を消します。

### ファイルの口は、ログインの控えも受ける

**録画をブラウザで観るようになったので、`<video>` が同じ口を取りに来ます**
([library.md](library.md#観るのはブラウザで))。`<video>` の再生に期限付きのリンクは
使わず (画面はログイン済みなので要らない)、ログインの控えをここでも
受けます (`auth.sessionMayRead`)。通る相手は増えず、
...
(プレイヤーの登録・ダウンロード) は期限付きリンクに置き換わって残っていません。起動ログで
しか受け取れないパスワードは流すと詰みます。自動で掛かる形は「設定を忘れた」と区別が
付きませんでした。閉じていれば閉じていると分かるほうを採りました。
...
ロールで決めるなら、アプリ登録の**アプリロール**を作って人に割り当て、`OIDC_GROUP`
にはそのロールの値を書きます。
...
手で作った Secret がどこにも書かれていない状態を無くすためです。クラスタを立て直したとき、
他は全部 ArgoCD がリポジトリから戻すのに、これだけは手順を思い出して打ち直すしかなくなる。
