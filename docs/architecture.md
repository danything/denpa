...
**映像・音声・字幕・データ放送まで入っています。** 残っているのは双方向
(通信系コンテンツ) だけで、どこまで入っているかも含めて
[stream.md](stream.md) にあります。焼き方・遅延の詰め方・WebSocket の作りもそちら。

録画済みのものを手元のプレイヤーで観るのは denpa の `/dav` です。こちらは別物で、denpa の
認証も削除も効きます。
