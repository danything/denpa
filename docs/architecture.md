...
| ライブ視聴 (放送中のものを観る。映像・音声・字幕・データ放送) | [stream.md](stream.md) |
...
配ります。読めないのは生TS (MPEG-2) だけで、それは追っかけ再生の道で観ます。

**境界は「電波を掴むところ」と「中身を読むところ」で切ってあります。** エージェントは
チャンネルを掴んで素の TS を流すだけで、NIT も SDT も EIT も解きません。局の選り分けも、
番組表の組み立ても、ロゴ拾いも denpa です ([agent.md](agent.md))。
...
TSは追記で開き、次の起動で `recoverOrphanedRecordings` が続きから録り直すので、
落ちても丸ごとは失いません。それでも居座るのは、入れ替わるまでの十数秒は
どうやっても落ちるからです。ArgoCD の同期は待てますが、放送は待ちません。

**待っている間も画面は開けます。** ここは2箇所で落ちていました。

1. **adapter-node が SIGTERM を受けたその場で listen を閉じる。** 居座っている間、
   Pod の中からも `127.0.0.1:3000` が拒否されていました。止まれの合図は
   ...
   **引き取りは1回では足りません。** あちらが登録するのは `build/index.js` の
   ...
   なので、最初の引き取りでは外すものがまだ無い。そのままだと両方が
   登録され、プロセスは生きたままポートだけ閉じます。実機で
   `/proc/net/tcp` に listen が1つも無く、前段が「no available server」を
   返しているのに、番組表は集まり続けていました。
...
録画していないときは即座に止まるので、普段の入れ替えは変わりません。
すぐ入れ替えたいときは `SHUTDOWN_WAIT=0` にすると、以前の「落ちて、
続きから録り直す」に戻ります。

Kubernetes の `terminationGracePeriodSeconds` と docker compose の
`stop_grace_period` は `SHUTDOWN_WAIT` より長くしておくこと。短いと待っている
途中で SIGKILL され、居座った意味が無くなります。

### エージェントも同じように居座ります

**denpa だけが待っても足りません。** TS を流しているのはエージェントなので、
そちらが先に消えれば掴んでいたストリームごと切れます。実機では、エージェントの
Pod を入れ替えただけで、始まって10秒の30分番組が丸ごと失敗しました。

エージェントも `SHUTDOWN_WAIT` (既定6時間) を見て、録画が終わるまで居座ります。
番組表・ロゴ・スキャンは切れたら取り直せばいいので待ちません
...
止めます。実測で、合図の直後からバイトが1つも進まなくなりました。denpa 側の
`takeOverSignals` と同じ理由です。

### それでも切れたときは掴み直します

居座りは「こちらから止めるとき」の話で、**電源やネットワークで切れることは
あります**。読んでいる最中に例外が飛んだときも、EOF と同じように掴み直して
終了時刻まで録り続けます (`recorder.ts`)。例外を外へ抜けさせていた頃は、
...
入れ替えのときにはノードの手元にあり、隙間は起動そのものだけになります。
...
(どちらが主かを決める仕組みが別途要る)。
録画をエージェント側に持たせれば denpa の入れ替えと録画が切り離れますが、
番組追従・スクランブル解除・保存名の付け方まであちらへ移すことになり、
「エージェントは中身を読まない」という切り分けが崩れます。**この道は選びません。**
...
何倍も掛かるので使いません。片方の arch が落ちたら名前は付けません。片方だけの
`develop` が出ていくより、前のものが残るほうがいい。中身の違いは Intel の QSV だけで、
...
何も変わっていないように見えて古い Pod が残るので、CI は Pod の注釈
...
リリースを作ると、焼き直さずにそのコミットの `sha-` へ版と `latest` を貼り足します
(`.github/workflows/release.yml`)。試したものとリリースしたものを別にしないためです。
...
いるので、貼ると入れて使っている人のところへ降ってしまいます。
`x.y.z` の形をしていないタグ (`nightly` など) には、版だと思って固定されないよう
版の名前も貼りません。

## ライブ視聴

**denpa 自身が配ります** (`/live`)。外のメディアサーバにも IPTV 向けのプレイリストにも
任せていません。視聴・追っかけ録画・チューナーの取り合いを1箇所で決めたいからです。
チューナーを取り合う相手 (録画・番組表・ロゴ・スキャン) が全部 denpa 側に居るので、
視聴だけ外に出すと優先度の決め方が2箇所に割れます。

任せる先もありません。エージェントは局も番組表も知らないので、IPTV 向けの
プレイリストと番組表 (m3u + XMLTV) を出せる相手が居ません。

映像・音声・字幕・データ放送・双方向 (通信系コンテンツ) まで入っています。
どこまで入っているか、焼き方・遅延の詰め方・WebSocket の作りは [stream.md](stream.md) にあります。

録画済みのものを手元のプレイヤーで観るには、テレビの VLC へ飛ばすか、
期限付きの再生リンクを貼ります ([library.md](library.md#手元のプレイヤーで観る))。
...
`helm.valuesObject` (インライン) を重ねて当てており、chart の使い方の実例として
