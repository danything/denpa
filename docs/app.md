...
| `/settings` | 録画のしかた(映像コーデック — **AV1・H.264 を複数選べる**。両方選ぶと1本を2ファイルで焼く。どちらも外すと「エンコードしない」/CMの扱い/CMの探し方/ロゴの当てにしかた/生TSを残すか/無料放送だけか)、通知先(Webhook)、ベーシック認証(パスワードの表示と作り直し)、データ放送(郵便番号・双方向の入切)、**テレビで再生 (VLC)** — 名前+IP+ポート+コーデックの行編集。並べていないと詳細にボタンは出ない。コーデックは AV1 を解けないテレビに H.264 や生TSを渡すためのもの、EPGStation からの引き継ぎ、**画面の高さを見る** (端末ごと。PWA にはアドレスバーが無く `?measure` を打てないため、入り口をここに置いてある)。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
