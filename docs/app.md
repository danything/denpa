...
| `DENPA_VERSION` | `dev` | 動いている版。**CI がイメージに焼き込む** (そのコミットから辿れる直近のリリースの札)。`dev` なら新しい版の知らせを出さない |
| `DENPA_RELEASES_URL` | GitHub の `releases/latest` | 新しい版を見に行く先。試験では偽物を指す |
...
| ヘッダー (どの画面でも) | **新しい版が出ていれば札を出す** (`v1.8.0 が出ています`。押せば GitHub のリリース)。サーバが 1 時間に 1 度、動いている版 (`DENPA_VERSION`) と GitHub の最新のリリースを見比べる (`server/update.ts`)。**リリースを消せば引っ込む** — 最新は消したあとの版になるので。閉じる口は置いていない (上げるか、消えるまで出ている) |
