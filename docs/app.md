...
CMの探し方・ロゴの当てにしかた・生TSを残すか・無料放送だけか・ベーシック認証・
データ放送の郵便番号がここです。
...
| `FPS_SURVIVE` | `0.45` | コマ数の実測の閾値。60コマ化→重複落とし後の生存率がこれ以下なら30コマ ([encode.md](encode.md#コマ数は本編の映像から測って決める)) |
...
| 画面 | 役割 |
| --- | --- |
| `/` | **予約と録画**を2ペインで並べる。**録画の行を押すと観る画面 (`/watch/<id>`) へ**、中身は「詳細」から。行の形はどの画面幅でも同じで、狭いところでは押すものが下へ回り込む。**行の頭にポスター** (`api/recordings/<id>/poster` = 焼いたときの `-poster.jpg`。無ければ枠だけ) で、テレビの画面でも文字だけより選びやすい。**途中まで観たものは「続き」の進捗バー** (`resume_ms` が残っている = まだ途中)。生TSを残しているときは大きさを両方出す (`43 MB (生TS 594 MB)`)。**番組名・シリーズ・副題・局で絞り込める** (GET なので URL に残る)。**300件で頭打ちのときはその旨を出す** (古いものは絞り込みで探す)。**録り逃した予約も残す** (放送が終わると消えていたので、チューナーが足りずに落とされたものに気付けなかった)。**どちらの行にも「何で立ったか」を出す** — ルール名がそのまま編集への入口で、録画の側は手動で録ったものもそう書く (ルールを消したあとは「ルール: (削除済み)」)。**エンコードの失敗ではダウンロードも録り直しも消さない** — 落ちたのは焼き直しのほうで、生TSは無事 (ただし**再生の印は出さない** — 生TSは MPEG-2 でブラウザが読めない) |
...
| `/chase/<id>` | **追っかけ再生** ([issue #16](https://github.com/danything/denpa/issues/16))。録画中の録画を頭から観る。一覧の録画中の行の「追っかけ」から。生TSはブラウザで読めないので、サーバが伸びるファイルを追い読みして**ライブと同じ器** (fMP4 → WebSocket → MSE) で焼き直す。チューナーは掴まない。シークバーは番組の全長で、右端 (いま録れているところ) は伸びていく — 手元に残っている範囲は即座に、外は `chase` の送り直し (ffmpeg 立て直し) で跳ぶ。視聴位置は数十秒おきにサーバへ写すので、焼き上がったあとの `/watch` が続きから拾う。焼き上がった録画で開くと `/watch` へ回す |
| `/watch/<id>` | **録画をブラウザで観る** ([library.md](library.md#観るのはブラウザで))。**タブレット以上は2段組で、左に映像・右に番組の中身** (ライブと同じ形・同じ幅 768px)、狭い画面では映像が上。指で触っている端末は**開いた時点で全画面**。**操作列は自前** — どこを押しても再生/一時停止 (指は操作列の出し入れ)、**左右の端を素早く2回で10秒** (送りのボタンは置いていない。PCは矢印キー)、**CMを自動で飛ばす**とチャプター送り、**観たその場で削除**、**続きから再生**。閉じる・**データ放送 (d)**・切り抜き・削除は**右端の縦の列**（データ放送を持つ録画でだけ d を出す。押すと右にリモコン）。番組の中身は**右に出たまま** (モーダルにしない — 映像の上に被さると観ながら読めない)。観られるのは**焼いたものだけ** (`?source=encoded`)。**字幕は既定で出す** (ライブと同じ)。放送どおりの絵 (PGS) をそのまま重ねる。**倍速**も選べる (ライブの追っかけと同じ並び)。**音声トラックも選べる** — 二カ国語・二重音声を焼いたものは両方入っているので、ブラウザが器から出せたときだけ切り替えを出す (`video.audioTracks`。出せない端末では出さない。ライブと違いサーバは焼き直さない) |
...
| `/settings` | 録画のしかた(映像コーデック — **AV1・H.264 を複数選べる**。両方選ぶと1本を2ファイルで焼く。どちらも外すと「エンコードしない」/CMの扱い/CMの探し方/ロゴの当てにしかた/生TSを残すか/無料放送だけか)、通知先(Webhook)、ベーシック認証(パスワードの表示と作り直し)、データ放送(郵便番号・双方向の入切)、EPGStation からの引き継ぎ、**画面の高さを見る** (端末ごと。PWA にはアドレスバーが無く `?measure` を打てないため、入り口をここに置いてある)。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
