...
| `/watch/<id>` | **録画をブラウザで観る** ([library.md](library.md#観るのはブラウザで))。**タブレット以上は2段組で、左に映像・右に番組の中身** (ライブと同じ形・同じ幅 768px)、狭い画面では映像が上。指で触っている端末は**開いた時点で全画面**。**操作列は自前** — どこを押しても再生/一時停止 (指は操作列の出し入れ)、**左右の端を素早く2回で10秒** (送りのボタンは置いていない。PCは矢印キー)、**CMを自動で飛ばす**とチャプター送り、**観たその場で削除**、**続きから再生**。閉じる・切り抜き・削除は**右端の縦の列**。番組の中身は**右に出たまま** (モーダルにしない — 映像の上に被さると観ながら読めない)。観られるのは**焼いたものだけ** (`?source=encoded`)。**字幕は既定で出す** (ライブと同じ)。放送どおりの絵 (PGS) をそのまま重ねる。**倍速**も選べる (ライブの追っかけと同じ並び) |
...
| `/settings` | 録画のしかた(映像コーデック — **「エンコードしない」もここ**/CMの扱い/CMの探し方/ロゴの当てにしかた/生TSを残すか/無料放送だけか)、通知先(Webhook)、ベーシック認証(パスワードの表示と作り直し)、データ放送(郵便番号・双方向の入切)、EPGStation からの引き継ぎ、**画面の高さを見る** (端末ごと。PWA にはアドレスバーが無く `?measure` を打てないため、入り口をここに置いてある)。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
...
| `--app-h` (CSS 変数) | **画面の高さ。単位ではなく測った値。** `app.html` の中で `documentElement.clientHeight` を写し、`+layout.svelte` が大きさの変わるたびに撮り直す。土台・番組表の表・番組詳細の窓がこれで高さを採る。`100dvh` は**実機の PWA (Android Chrome) で画面より 56px 高くなった** — 56px は Chrome for Android のアドレスバーの高さそのもの。組み上がりを待って測ると**その前に一度はみ出したものが描かれる**ので、色と同じく `app.html` で当てる。当て (SSR・JS 切) だけ `100dvh` |
