...
| `/settings` | 録画のしかた(映像コーデック — **「エンコードしない」もここ**/CMの扱い/CMの探し方/ロゴの当てにしかた/生TSを残すか/無料放送だけか)、通知先(Webhook)、ベーシック認証(パスワードの表示と作り直し)、データ放送(郵便番号・双方向の入切)、EPGStation からの引き継ぎ。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
