...
| `/api/recordings/<id>/share` | 期限付きの再生リンクを作る (share.ts)。テレビへの飛ばしもこれを使う。**リンクの尻は `/file/<番組名>.mkv`** (生TS の名指しなら `.ts`) — テレビの VLC は URL の最後の区切りを見出しにするので (入れ物の title は見出しに効かない)、名前をここに乗せる。サーバはその段を読み捨てる (`file/[name]/+server.ts`) |
