...
保存先への配置までを受け持つ。
出来上がった mkv はブラウザ (`/watch`) か、denpa が配るものを外部プレイヤー (テレビの VLC など) で観る。削除は手動。
...
| `src/lib/server/webhook.ts` | 録画の節目 (開始・完了・失敗)、エンコードの完了・失敗、録画の外側の変事 (ディスク残量わずか・チューナーに繋がらない/繋がった) を外部へ通知する |
...
`recordings.state` は SQLite の**生成列** (`schema.RECORDING_STATE`) で、書き込もうとすると
SQLite が拒むので、事実と状態が食い違いません。文字列で別に持っていた頃は、
エンコードの失敗が録画の失敗として書き込まれ、中身のある生TSを持ったまま
再生もダウンロードもできなくなっていました。予約側も `recording` / `done` /
`failed` を書き写していたので、録画が失敗しても予約が録画中のまま残ることがありました。

チューナーの本数を数えるときは、番組の時刻ではなく前後マージンを足した
「実際に掴んでいる区間」で重なりを見ます。22:00 終了と 22:00 開始は実際には重なるので、
揃えないと予約表では通るのに実行時に録り逃します。

数えるのは**種別 (GR / BS / CS) ごと**で、同時に開いている異なる物理チャンネルの数が
その種別のチューナー本数を超えたときだけ競合です (`conflict.ts` の `assign`)。
ルール画面のプレビューも同じ物差しです (`contending`)。時間の重なりだけを
出していた頃は、衛星の番組に地上波の番組が競合として並び、地上波チューナーが2本
あって録れる組にも印が付いていました。本数が分からないとき (エージェントに繋がらない
とき) はどちらも何も言いません。

**丸ごと入らないからといって、丸ごと捨てません。** 実機では、30分番組4本のうち
15分だけ3チャンネル要る形の取り合いが出ました。空いている一番長い区間だけ掴みます
(`assign` の「入るところまで録る」)。採るのが優先度の高い順なので、切られるのは必ず
優先度の低いほう。譲った区間は `record_from` / `record_to` に残り (番組の時刻は動かさない)、
予約一覧は「頭 15分 が欠けます」、録画一覧は「欠けています」と言います
(`format.clipNote`)。マージンだけ削られたぶんは言いません。隣り合う番組は
マージンぶんが必ず重なるので、数えるとほとんどの録画に札が付きます。

**番組はマージンに勝ちます。** 22:00 終了 → 22:00 開始の隣り合いは、前の
終了マージンと後の番組そのものの取り合いで、保険であるマージンが譲ります。
丸ごと競合にしていた頃は、隣り合っているだけで後ろが録れませんでした。

録画中にプロセスが落ちたら、次の起動でまだ放送中なら録り直します。生TSは追記で
開くので、落ちるまでの分は残ります。放送が終わっていれば失敗に倒します。

**掴んでいる者が居ないまま終了時刻を過ぎた録画は、見回りが畳みます** (`failStrayRecordings`、
猶予1分)。起動時の拾い直しだけだと、録り直しに行った先でまた殺されたとき
(ノードごと落ちる・Pod を強制終了する)、次の起動まで誰も畳みません。畳まれない行は
予約の一覧に「録画中」で居座り、録画の一覧には出てきません (どちらの振り分けも
`recordings.state = 'recording'` で決まる)。生TSは消さないので、録れていた分は詳細から焼き直せます。

## 環境変数

**外から差し替える理由があるものだけ。** 相手の居場所と、テストで詰めたい間隔です。
検出のしきい値やサムネイルの大きさまで環境変数にしていた頃は、誰も触らないのに
既定値の出どころが追いにくくなるだけでした (`src/lib/server/config.ts` に直に書いてあります)。

画面から変えたいものは設定画面 (`src/lib/server/settings.ts`)。何があるかは
[画面](#画面)の `/settings` の行に。

本番で差し替えているのは、既定値では決められないものだけです
...
に重なる)。置き場所やエージェントの居場所は既定値がそのままあの構成なので書いていません。
同じ値を書き写すと、片方だけ直したときにどちらが効いているのか分からなくなります。
...
| 画面 | 役割 |
| --- | --- |
| `/` | **予約と録画**を2ペインで並べる。録画の行を押すと観る画面 (`/watch/<id>`) へ、中身は「詳細」から (生TSがある間はエンコード中・エンコード失敗でも追っかけ再生 `/chase/<id>` へ。押せないのは録画失敗・削除済み・録り逃しで、その行は行そのものが詳細の入口)。めったに押さないもの (ダウンロード・再生リンク・再エンコード) は詳細の「その他…」に畳む。行の形はどの画面幅でも同じで、狭いところでは押すものが下へ回り込む。行の頭にポスター (`api/recordings/<id>/poster` = 焼いたときの `-poster.jpg`。無ければ枠だけ) を置き、テレビの画面でも選びやすくする。途中まで観たものは「続き」の進捗バー (`resume_ms` が残っている = まだ途中)。置いてあるものは全部の大きさを出す (`43 MB (H.264 71 MB、生TS 594 MB)`)。主 (AV1) のぶんだけでは、消していいのか・どれだけ空くのかが読めない。番組名・シリーズ・副題・局で絞り込める (GET なので URL に残る)。300件で頭打ちのときはその旨を出す (古いものは絞り込みで探す)。録り逃しは録画の一覧に出す (録画の結果なので。以前は黙って消え、チューナーが足りずに落とされたものに気付けなかった)。確かめ終わったら2回押しで消せる。どちらの行にも「何で立ったか」を出す。ルール名がそのまま編集への入口で、手動で録ったものもそう書く (ルールを消したあとは「ルール: (削除済み)」)。エンコードの失敗ではダウンロードも録り直しも消さない。落ちたのは焼き直しのほうで、生TSは無事 (行を押せば追っかけ再生で観られる) |
| `/guide` | 番組表(グリッド)と番組検索。マスはジャンルごとに色を変える。詳細から予約・取消と、焼けていれば録れたものをそのまま観られる。いま流れている番組には「視聴」が出て、その局でライブ画面が開く。音声は**放送が付けた名前**で出す (「主音声ステレオ」「解説ステレオ」)。符号だけだと解説放送で同じ札が2つ並ぶ |
| `/live` | **いま流れているものを見る** ([stream.md](stream.md))。左に映像、右にチャンネル。前回見ていた局から開く (覚えていなければリモコン番号のいちばん若い局。番組表の「視聴」から来たときはその局)。局の並びは番組表と同じ。操作列は自前 (備え付けは使わない) で、止めた所から見られる (追いかける速さを選べ、追いついたら自分でライブに戻る)・音声を選べる・焼き方 (H.264 / AV1) を見ながら選べる・字幕を出せる・いまの1コマを字幕ごと切り抜ける・切り替えの間は前の絵を貼る。d と切り抜きは右上の縦列 (観る画面と同じ場所・同じ並び)。右上の録画ボタンで、いま観ている番組の録画が始まる (手動予約と同じ道)。放送そのまま (MPEG-2) はブラウザに復号器が無いので出せない ([stream.md](stream.md) §5.5)。データ放送も出せる (d ボタン。指で押せるリモコンが右に並ぶ)。Hybridcast は行き先だけ出す (別のタブで開く。動かさない) |
...
| `/rules` | 自動予約ルールの一覧と作成。**優先度は予約どうしを比べる数** (0〜9。チューナーが足りないとき大きいほうを残す。チューナー画面の「掴む強さ」とは別の物差し)。「この条件で録れる番組」に予約済みのものも競合も同じ表で出す (1件ずつ取り消せる)。行を押すと予約一覧と同じ番組詳細が出て、その1本だけ予約できる (条件を詰めている途中で録っておきたい番組に行き当たっても、番組表で探し直さずに済む)。いま流れていれば「視聴」も出る (番組表の詳細と同じ判断。`epg.watchableServices`)。取り消しはここに出さない。行のほうに置いてあり、予約済みの行は状態の札になる。「何が録れるか見る」は GET でこの画面に戻ってくるので、条件は全部 URL に乗る。受け取る側が読み落とすと、見ただけのつもりが保存で書き換わる (優先度で実際に起きた) |
| `/tuners` | **チューナーの設定** (本数・デバイス・受けられる種別・LNB・無効化。書かなければ自動検出)、チャンネルスキャン (途中で中断でき、録画中でも実行できる)、チューナーの空きと何を掴んでいるか、取れているチャンネル (番組表の集まり具合つき)、エージェントとカードリーダー (リーダーごとの表: カードの番号・使用中/予備・そのカードで解いているチューナー) と局ロゴの状態 (局ロゴをいますぐ取りに行く: 地上波も衛星もチューナー2つで。進み具合が出る)。チューナーを掴んでいる相手は用途で出す (「録画: 番組名」「番組表 (T16)」「局ロゴ収集 (T16)」)。CM検出用のロゴもここで、局ごとに覚えた絵を出し、位置を教える・捨てて覚え直す |
| `/settings` | 録画のしかた(映像コーデック — AV1・H.264 を複数選べる。両方選ぶと1本を2ファイルで焼く。どちらも外すと「エンコードしない」/CMの扱い/CMの探し方/ロゴをどれだけ当てにするか/コマ数を映像から決めるか/生TSを残すか/無料放送だけか)、GPU (見つかった口ごとに QSV / VA-API × コーデックの印。使えるものに自動で付く。「確かめ直す」で再探索)、通知先(Webhook)、データ放送(郵便番号・双方向の入切)、テレビで再生 (VLC) — 名前+IP+ポート+コーデックの行編集。登録が無いと詳細にボタンは出ない。コーデックは AV1 を解けないテレビに H.264 や生TSを渡すためのもの、EPGStation からの引き継ぎ、画面の高さを見る (端末ごと。PWA にはアドレスバーが無く `?measure` を打てないので、入り口をここに置く)。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
| `/api/recordings/<id>/file` | 録画ファイル。Range 対応。**エンコード済みがあればそちら、無ければ生TS。焼き直している間は生TS** (始める前に前のものを消すので、その間は保存先に何も無い)。`?source=ts` / `?source=encoded` / `?source=alt` (H.264) で名指しもできる。詳細のダウンロードは、残っているぶんだけこれで口を分けて出す ([library.md](library.md)) |
| `/api/recordings/<id>/frame?at=<秒>` | 録画から1コマ (JPEG)。ロゴの位置を指定するときに使う (既定で右上を 16:9 のまま拡大、覚えてある枠は掴んで動かせる) |
| `/api/recordings/<id>/poster` | 焼いたときに動画の隣へ置いた `-poster.jpg` を返す (一覧のサムネイル用)。`frame` と違い ffmpeg を起こさず読んで返すだけ。無ければ 404 |
...
| `/api/recordings/<id>/share` | 期限付きの再生リンクを作る (share.ts)。テレビへの飛ばしもこれを使う。**リンクの尻は `/file/<番組名>.mkv`** (生TS の名指しなら `.m2ts`)。テレビの VLC は URL の最後の区切りを見出しにする (入れ物の title は効かない) ので、名前をここに乗せる。サーバはその段を読み捨てる (`file/[name]/+server.ts`) |
| 画面の高さの採り方 | **単位で言い当てない。** `app.css` が `html, body` に高さを与え、土台も番組詳細の窓もそこから `%` で降りる (`min-h-full` / `md:h-full`)。`100vh` も `100dvh` も実機の PWA (Android Chrome) で画面より 56px 大きく出た。56px は Chrome for Android のアドレスバーの高さそのもの。`%` なら JS も測り直しも要らず、描く前から正しい。`%` が届かないのは番組表の表だけ (畳まれる幅では途中の入れ物に高さが決まらない) で、そこは3つの単位のうち見えている範囲を超えない `svh` を使う |
...
| ヘッダー (どの画面でも) | **新しい版が出ていれば札を出す** (`v1.8.0 が出ています`。押せば GitHub のリリース)。サーバが 1 時間に 1 度、動いている版 (`DENPA_VERSION`。リリースのイメージにだけ入っている) と GitHub の最新のリリースを数で比べる (`server/update.ts`)。develop のイメージは版を持たないので出さない (main を追う限りリリースより常に先)。リリースを消せば、最新が消したあとの版になるので引っ込む。閉じる口は置いていない (上げるか、消えるまで出ている) |
| `/manifest.webmanifest` | PWA のマニフェスト。来た名前で表示名が変わるので静的ファイルではない。認証を掛けていない口の1つ (ブラウザが資格情報を付けずに取りに来るため。他は `/login*`・`/logout`・`/api/health`。`auth.ts` の `OPEN_PATHS`) |
