...
| `src/lib/ts/ass.ts` | 文字の字幕 (ASS) を読み、整えて、WebVTT に直す。**ルビは座標から掛かる字を探して本文に畳み込む** (DOM を触らない) |
...
| `src/lib/components/player/` | 映像の上に置くもの。**ライブと観る画面で共通** (アイコン・重ねボタン・操作列・出し入れの決め方) |
...
| `/watch/<id>` | **録画をブラウザで観る** ([library.md](library.md#観るのはブラウザで))。**タブレット以上は2段組** (左に番組の中身、右に映像)、狭い画面では映像が上。指で触っている端末は**開いた時点で全画面**。**操作列は自前** — どこを押しても再生/一時停止 (指は操作列の出し入れ)、**左右の端を素早く2回で10秒**、CM飛ばし (チャプター送り)、**観たその場で削除**、**続きから再生**。番組の中身は**左に出たまま** (モーダルにしない — 映像の上に被さると観ながら読めない)。観られるのは**焼いたものだけ** (`?source=encoded`)。**字幕は持っている録画でだけ出せる** — 入れ物の中の PGS は絵なので、動画の隣に置いた文字のほうを WebVTT に直して渡す |
| `/api/recordings/<id>/resume` | どこまで観たかを覚える (POST)。**端末ではなくサーバに置く**ので別の端末でも続く。末尾まで観たものは忘れる |
| `/api/recordings/<id>/subtitle.vtt` | 字幕を WebVTT で返す。元は動画の隣の `<動画名>.ja.ass` ([encode.md](encode.md#文字の写しを隣に置く))。無ければ 404 (画面はボタンを出さない) |
