...
| 画面 | 役割 |
| --- | --- |
| `/` | **予約と録画**を2ペインで並べる。**録画の行を押すと観る画面 (`/watch/<id>`) へ**、中身は「詳細」から (観られない行 [失敗・エンコード中・削除済み・録り逃し] は行そのものが詳細の入口で、ボタンは置かない)。めったに押さないもの (ダウンロード・再生リンク・再エンコード) は詳細の「その他…」に畳む。行の形はどの画面幅でも同じで、狭いところでは押すものが下へ回り込む。**行の頭にポスター** (`api/recordings/<id>/poster` = 焼いたときの `-poster.jpg`。無ければ枠だけ) で、テレビの画面でも文字だけより選びやすい。**途中まで観たものは「続き」の進捗バー** (`resume_ms` が残っている = まだ途中)。生TSを残しているときは大きさを両方出す (`43 MB (生TS 594 MB)`)。**番組名・シリーズ・副題・局で絞り込める** (GET なので URL に残る)。**300件で頭打ちのときはその旨を出す** (古いものは絞り込みで探す)。**録り逃しは録画の一覧に出す** (「これから録るもの」ではなく録画の結果なので。以前は黙って消え、チューナーが足りずに落とされたものに気付けなかった)。確かめ終わったら2回押しで消せる。**どちらの行にも「何で立ったか」を出す** — ルール名がそのまま編集への入口で、録画の側は手動で録ったものもそう書く (ルールを消したあとは「ルール: (削除済み)」)。**エンコードの失敗ではダウンロードも録り直しも消さない** — 落ちたのは焼き直しのほうで、生TSは無事 (ただし**再生の印は出さない** — 生TSは MPEG-2 でブラウザが読めない) |
...
| `/settings` | 録画のしかた(映像コーデック — **AV1・H.264 を複数選べる**。両方選ぶと1本を2ファイルで焼く。どちらも外すと「エンコードしない」/CMの扱い/CMの探し方/ロゴをどれだけ当てにするか/生TSを残すか/無料放送だけか)、通知先(Webhook)、データ放送(郵便番号・双方向の入切)、**テレビで再生 (VLC)** — 名前+IP+ポート+コーデックの行編集。登録が無いと詳細にボタンは出ない。コーデックは AV1 を解けないテレビに H.264 や生TSを渡すためのもの、EPGStation からの引き継ぎ、**画面の高さを見る** (端末ごと。PWA にはアドレスバーが無く `?measure` を打てないため、入り口をここに置いてある)。**広い画面では2列**で、幅いっぱいのカードは置かない (2列の下に帯が続くと、どこまでが左の列の続きか読めない) |
