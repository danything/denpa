...
| 写していないもの | 単体ページ・サーバ・自前再生 (入口から辿れないもの) |
...
**それ以外は1バイトも違いません。** 一時期 `client/interpreter/js_interpreter.ts`
だけ denpa が書いていました — 向こうの README が「未使用」と言っている道なのに、
Vite では読み込んだ時点で転ぶファイルだったためです。**上流が 2026-08 に
ファイルごと消した**ので、いまは写しが完全に機械的です。
...
借りものを書き換えずに済ませるための細工は、**いまは1つも要りません**。
...
写す顔ぶれは、**denpa が呼ぶ入口から `import` を辿って決めます**
(`bml_browser` / `content` / `entity_parser` / `ws_api`)。上流が依存を
増やしても減らしても、写し直せば顔ぶれのほうが付いてきます。

**前は「いま置いてあるのと同じ顔ぶれ」に固定していました。** 上流が依存を
増やしたときに `bun run check` が「そんなファイルは無い」で落ち、それが
「人が見て決め直せ」という札になる、という考えでした。**札の出し方が
ビルド失敗なのが良くありませんでした** — 2026-08 の追従で上流が4ファイル
増やし (`util/logger` ほか)、**PR が丸ごと組めない状態で出ました**。増えたことは
分かっても、中身を読む前に手を動かすことになります。

いまは同じ出来事が**説明の「増えた」の一覧**になります。札は消えていません —
むしろ、増減がそこに並ぶぶん目につきます。顔ぶれが機械的に決まることは
確かめてあります (固定していた 44 ファイルと、入口から辿った閉包は完全に一致)。
