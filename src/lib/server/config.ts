...
export const config = {
     ...
     * 「同じ絵が並ぶ素材 (アニメ・フィルム)」= 30コマで出す。5窓の中央値で見る
     * (encoder.FPS_POINTS)。本番の実測 (2026-08、5窓の中央値): 30コマのアニメ 17〜28%、
     * 毎秒 30 枚が違う絵のアニメ 48%、実写 (CM の窓を代わりに) 60〜79% (下位 1/4 でも
     * 53% 以上)。**50% はその間** — 45% だった頃は 48% のアニメが 60 に倒れていた。
     * 実写側の余裕は 3〜10 ポイントしか無いので、これ以上は上げないこと
     */
