...
function videoArgs(codec: LiveCodec): string[] {
     ...
     *
     * ## 山だけは抑える (`-maxrate` / `-bufsize`)
     *
     * 上の 8.2 Mbit/s は**動きの少ない場面での値**。実測で、**高校野球の中継を
     * 25分受けたときの平均は 14.5 Mbit/s** だった (素の WebSocket で 2.3GB)。
     * 芝と土と観客席が一面で動くので、`crf 23` は素直にそのぶん出す。
     *
     * **宅内でも無線は割り込む。** 実機で「一瞬止まって遅延が増えていく」が
     * 出ており、送り出す側は入口 (TLS) まで含めて途切れ0、**AV1 (6.6 Mbit/s)
     * に替えると目に見えて減る** — 流量の話だと分かる。
     *
     * `-maxrate` を付けると x264 は VBV を回すので、**山だけが削れて**平らな
     * 場面は今までどおりになる。`-bufsize` は 1秒ぶん — 大きく取ると山を
     * ならす代わりに、ならしている間の遅れがそのまま出る
     */
    return [
        '-c:v',
        'libx264',
