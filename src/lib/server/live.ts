...
function videoArgs(codec: LiveCodec): string[] {
     ...
     * 一時ここに `-maxrate 8M -bufsize 8M` を入れて、山を 8.4 Mbit/s まで
     * 削っていた — 実機の「一瞬止まって遅延が増えていく」が流量の話だと
     * 見立てたため (AV1 の 6.6 Mbit/s に替えると目に見えて減っていた)。
     *
     * **見立てが違った。** 削ったあとも実機では変わらず、手元のブラウザで
     * 入口込みで測ると **8.4 Mbit/s のまま8分で10回、ちょうど60秒ごと**に
     * 止まっていた。周期がここまで揃うのは電波の混み具合ではありえない —
     * 原因は受け側の貯め方で ([ts/pacing.ts](../ts/pacing.ts) の `nextTarget`)、
     * **焼き方を一切触らずに 10回 → 3回、以後6分間0回**になった。
     *
     * 山を削る理由が無くなったので外した。VBV を回すと動きの多い場面だけ
     * 絵が崩れるうえ、`-bufsize` は `-maxrate` と対でしか効かない。
     * **細い経路のためなら AV1 を選ぶ道がある** (`LIVE_CODECS`) — あちらは
     * 削るのではなく、同じ絵を小さく詰める
     */
