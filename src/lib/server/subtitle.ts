 ...
 * ## 文字の写しは作りません
 *
 * 同じ字幕を文字でももう1本取り出して、動画の隣に `.ja.ass` として置いていた
 * (`buildText`)。**置く理由が無かった。** 拾う相手として挙げていた Kodi は
 * **PGS をそのまま出せる**ので、代わりではなく「選べるもう1本」でしかなく、
 * そのために焼くたび 2〜9秒と動画1本ぶんの場所を払っていた。
 *
 * denpa の観る画面も絵のほうを使う — ブラウザに PGS の復号器はないが、
 * denpa 自身が解けば済む (`api/recordings/<id>/captions.sup`)。
 *
 * 文字にすると落ちるものもあった (左右の位置・背景の箱・外字・ルビ)。
 * **放送どおりに出せるのは絵のほうだけ**で、文字は最初から劣る側だった。
 */
