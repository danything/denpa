...
VLC のリモートアクセスを有効にしておくと、録画詳細の「テレビで再生」から
**いま開いている端末が**テレビへ直接飛ばします — 家のテレビは設定に居場所を書いて
ボタン一発、出先のテレビはIPをその場入力 (初回だけテレビに出る6桁コードで
その端末とペアリング)。「再生リンク」(24時間で切れるURL) をコピーして
どのプレイヤーにでも貼れます。**字幕はそのまま出ます** —
