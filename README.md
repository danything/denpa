...
> **ブラウザでは字幕が出ません。** 焼いたものに入っている字幕は文字ではなく
> **絵** (PGS) で、ブラウザに復号器がありません。字幕が要るときは落として
> VLC などで観てください ([docs/library.md](docs/library.md#観るのはブラウザで))。

**Kodi** から観るときは `/dav` を WebDAV サーバーとして追加します
(Android TV・Fire TV も同じ)。
