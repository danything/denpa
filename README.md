...
**いま開いている端末が**テレビへ直接飛ばします — 家のテレビは設定に並べて
ボタン一発、出先のテレビはIPをその場入力すれば次からボタンになります
(初回だけテレビに出る6桁コードでペアリング)。「再生リンクをコピー」(24時間で切れるURL) で、どのプレイヤーに
でも貼れます。字幕は入れ物の中に入っているのでそのまま出ます
([docs/library.md](docs/library.md#手元のプレイヤーで観る))。

## 誰を通すか

**何も設定しなくても、掛かった状態で上がります。** 初回の起動でベーシック認証を
その場で作り (ユーザー名 `denpa` 固定・パスワードは自動生成)、起動のログに1度だけ
出します。以降は設定画面にそのまま出ています。

- **家の中の端末に毎回入れさせたくない** — `TRUSTED_NETWORKS=10.10.0.0/16` のように
  通す網を書けます (テレビの VLC に資格情報を入れずに使わせるのもこれ)
- **外から使うなら OIDC を。** ベーシック認証だけでインターネットに晒さないで
  ください。`OIDC_ISSUER` など3つ渡すと画面のほうがログインに替わります
- **パスワードを流してしまったら**、DBから直に読めます
