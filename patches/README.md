# ffmpeg に当てている直し

Dockerfile の `ffmpeg` 段で、`./configure` の前に `patch -p1` で当てています。
**上流に投げるつもりのものだけ**を置きます — 手元の都合で ffmpeg の挙動を
変えたくなったら、まず denpa 側で済ませられないかを考えます。

## `ffmpeg-aribcaption-clear.patch`

**字幕が消えずに出っぱなしになるのを直します。**

放送は字幕の区切りごとに、**CS (画面消去) だけを載せた字幕文**を送ってきます
(実機の BS-TBS で確認。`1f 20 00 00 01 0c` の `0c` が CS)。これを受けた
libaribcaption は `region_count == 0` の字幕を返します。

ffmpeg のラッパは、

- **`-sub_type text`** … `""` の rect を1つ作って返す → 前の字幕が消える
- **`-sub_type bitmap`** … 描くものが無いので `0` を返す →
  **`*got_sub_ptr` が立たず、字幕が終わったことが誰にも伝わらない**

denpa は絵で受け取る (`server/captions.ts`) ので後者です。しかも直前の字幕は
`wait_duration` が無期限で `end_display_time = UINT32_MAX` になっているため、
**次の字幕が来るまで永久に消えません**。実機の録画では22分35秒間出たままでした。

生放送 (`server/live.ts`) も焼き込み (`server/subtitle.ts` の PGS) も同じ
デコーダを通るので、**ここ1箇所で両方直ります**。

**投げ先は ffmpeg (ffmpeg-devel) です。** libaribcaption 自身は
`region_count == 0` の字幕をちゃんと返していて、それを捨てているのは
ffmpeg 側のラッパなので、あちらに出しても直せません。

同じ100秒を通した実測 (数字は出てきた PNG のバイト数。10609B が全部透明):

```text
当てる前   … 64.82s 45790B   66.98s 45790B ← ここに「消せ」が来ている
              69.50s 45790B   74.77s 45790B   79.79s 45790B   84.82s 45790B
              89.84s 45790B   94.84s 45790B   99.86s 45790B  … 100秒間出っぱなし
当てた後   … 64.82s 45790B   66.98s 10609B ← 消えた
              67.85s 10609B   70.09s 10609B   75.11s 10609B   80.13s 10609B
              85.15s 10609B   90.16s 10609B   95.18s 10609B  … 透明のまま
```

## `ffmpeg-sched-overflow.patch`

**長い録画を 60コマで焼くと、20〜25分あたりで音声が黙って終わるのを直します**
(ffmpeg 9.0 / 9.0.1。8.1 までは起きない。2026-08-17 に実機の 2 時間の映画で
AV1 版 23:28・H.264 版 22:07 で音声トラックが終わり、エラーは 1 行も出なかった)。

ffmpeg 9 の CLI (`fftools/ffmpeg_sched.c`) は、先に進みすぎたデコーダを止めている間も
デマルチプレクサが詰まらないように、パケットを**デコーダごとの溢れ用 FIFO**
(`SchDec.overflow`) に溜めます。この FIFO は AVFifo の自動拡張で、上限が
**1 MB ぶんのポインタ = 131,072 個**。映像のエンコードが重くて音声デコーダが
長く止められると (60コマの映像 + 音声は軽い)、そこに当たって書き込みが `ENOSPC`
で失敗し、デコーダスレッドは負の戻り値をすべて EOF と見なして
「Decoder thread received EOF packet」で終わります。残りの音声は捨てられます。

実測で数が合います: 読んだ音声パケット 197,716 − 復号した 66,641 = 131,075。

直しは「満杯なら、止められていても最古のパケットを渡す」。厳密な同期より
メモリが有限であることを優先します。

書き込みは `AV_CONTAINER_FIFO_FLAG_REF` (参照を増やす) で行います。
`av_container_fifo_write` はプール側のオブジェクトへの移動を **FIFO への書き込みより先に**
するので、移動 (フラグ 0) で `ENOSPC` に当たると渡したパケットが空になって失われます。
最初の版はそうなっていて、FIFO が満杯になった 1 時間 10 分あたりから 5 個に 1 個の
音声パケットが抜けて途切れ途切れになりました。

当てた 9.0.1 で実機の 2 時間の映画を通しで焼くと (x264 ultrafast)、音声が最後まで
10 分ごとに 30,000 パケットずつ揃い、60 ms 超の途切れは 0 でした
(当てる前は 20〜25 分で終わる)。

**投げ先は ffmpeg-devel です。** master (2026-08 時点) にも同じ作りが残っています。
